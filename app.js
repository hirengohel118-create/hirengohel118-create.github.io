// Vamika Estate CRM - app.js (updated to send schedules to service worker)
// Original app logic remains intact; I only inserted schedule-message code where leads are saved.
...
(() => {
  ...
  setTheme(theme);
  // expose a global setter so external index.html can toggle theme
  ...
  leadForm.addEventListener('submit', (e) => {
    ...
    if(id) {
      state.leads = state.leads.map(l => l.id === id ? {...l,name,phone,req,note,follow} : l);
      // If updated lead has follow time, schedule again below
    } else {
      const newLead = { id: 'l_'+Date.now(), name, phone, req, note, follow, status:'new', created:Date.now() };
    ...
    // --- NEW: send schedule to service worker when follow time is set (and only if follow exists) ---
    const scheduledLead = state.leads.find(l => l.name === name && l.phone === phone && l.follow === follow);
    if (scheduledLead && scheduledLead.follow) {
      // If Notification permission not granted, ask (graceful)
      if (Notification.permission !== 'granted') {
        try { Notification.requestPermission(); } catch(err) {}
      }
      // Send schedule to active service worker
      if (navigator.serviceWorker && navigator.serviceWorker.controller) {
        navigator.serviceWorker.controller.postMessage({
          type: 'schedule',
          id: scheduledLead.id,
          name: scheduledLead.name,
          time: scheduledLead.follow
        });
  ...
  // listen for messages from SW (optional useful feedback)
  if (navigator.serviceWorker) {
    navigator.serviceWorker.addEventListener('message', (evt) => {
      // Currently we don't need to do much, but keep for debugging/feedback
      // evt.data = { type: 'fired', id: 'l_...' }
      if (evt.data && evt.data.type === 'fired') {
        // mark as notified in state if exists
        const id = evt.data.id;
        const lead = state.leads.find(x => x.id === id);
        if (lead && !lead.lastNotified) {
          lead.lastNotified = Date.now();
          saveState();
          renderLeads();
          renderToday();
        }
  ...
  // helper used by index.html to set theme externally
  function setTheme(t){ document.body.classList.remove('light','dark'); document.body.classList.add(t); }

  // rest of code (already included above) continues unchanged...
  // The rest of functions are already above in this file (we kept original code)
})();
